...
FROM (
	...
	FROM (
		SELECT
			...
			, mn.mn_name
			, fi.food_genus_id
			, sum( (mn.mn_value / 100 * amount_consumed_in_g) )/(select afe_total from survey_member_count smc where smc.survey_id = h.survey_id)  AS mn_consumed_per_day
        ...
        JOIN survey ON h.survey_id = survey.id
