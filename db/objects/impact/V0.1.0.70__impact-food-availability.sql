CREATE TABLE IMPACT_FOOD_AVAILABILITY (
	scenario             text REFERENCES IMPACT_SCENARIO (ID)
	, commodity          text REFERENCES IMPACT_COMMODITY (ID)
...
COMMENT on column impact_food_availability.commodity IS 'Commodity';
COMMENT on column impact_food_availability.country IS 'Country';
COMMENT on column impact_food_availability.year IS 'Year';
COMMENT on column impact_food_availability.scenario IS 'Socioeconomic scenario';
