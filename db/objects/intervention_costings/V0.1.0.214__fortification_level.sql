...
COMMENT ON TABLE fortification_level IS 'Table to store data which form a premix calculation for specific intervention which itself has a specific food vehicle and intervention type. These data contribute to the overall intervention cost.';
...
COMMENT ON COLUMN fortification_level.year                    IS 'For which year these fortification levels apply'; -- TODO - I don't think we need this column as premix calcs are not year-specific?
